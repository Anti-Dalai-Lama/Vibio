...
namespace Vibio.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(Vibio.Models.LoginViewModel model, string returnUrl) //0 references
        {
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)//THERE WE WILL GO IF U DONT HAVE AN ACCOUNT!!! из 136 строки
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new Vibio.Models.ApplicationUser { UserName = model.Name, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        Response.Cookies.Add(cookie);
                        using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["RemotedVibioDB"].ConnectionString))
                        {
